...
class NCEFunction(Function):
    @staticmethod
    def forward(self, imgs, y, memory, idx, params):
        K = int(params[0].item())
        T = params[1].item()
        Z = params[2].item()
        ...
        # inner product
        out = torch.bmm(weight, imgs.data.resize_(batchSize, inputSize, 1))
        out.div_(T).exp_() # batchSize * self.K+1
        imgs.data.resize_(batchSize, inputSize)
        ...
        self.save_for_backward(imgs, memory, y, weight, out, params)
...
class NCEAverage(nn.Module):
    ...
    def __init__(self, gpu, inputSize, outputSize, K, T=0.07, momentum=0.5, Z=None):
        super(NCEAverage, self).__init__()
        self.nLem = outputSize
        self.unigrams = torch.ones(self.nLem)
        self.multinomial = AliasMethod(self.unigrams)
        self.multinomial.to(get_device(gpu))
        self.K = K
    ...
    def forward(self, imgs, y):
        ...
        out = NCEFunction.apply(imgs, y, self.memory, idx, self.params)
        return out
