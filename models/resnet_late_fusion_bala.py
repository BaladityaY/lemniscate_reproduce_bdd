...
class ResNet(nn.Module):
    ...
    def _make_layer_concat(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes+3, planes * block.expansion, #steering concat
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes+3, planes, stride, downsample)) #steering concat
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes))
    ...
    def forward(self, imgs, steering):
        ...
        #steering = steering + Variable(torch.Tensor([2]).double()).expand(steering.size())
        #steering = (steering * Variable(torch.Tensor([.25]).double()).expand(steering.size())).int()
        #print('steering size: {}'.format(steering.size()))
        #print('steering vals: {}'.format(steering))
        
        # ORIGINAL WAY OF ADDING STEER
        #steering_one_hots = torch.zeros(7, steering.size()[0], steering.size()[1])
        steering = steering.view(steering.size()[0],-1).expand(28,28,-1,-1).permute(2,3,0,1)

        #print('steering size: {}'.format(steering.size()))
        #print('imgs size: {}'.format(imgs.size()))

        imgs = torch.cat((imgs, steering), dim=1)
        
        imgs = self.layer3(imgs)
        imgs = self.layer4(imgs)
        print('imgs before avgpool shape: {}'.format(imgs.shape))
        imgs = self.avgpool(imgs)
        print('imgs before view shape: {}'.format(imgs.shape))
        imgs = imgs.view(imgs.size(0), -1)
        print('imgs after view shape: {}'.format(imgs.shape))
        imgs = self.fc(imgs)
